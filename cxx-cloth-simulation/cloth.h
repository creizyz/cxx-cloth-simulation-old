...
struct Cloth
{
  ...
  Cloth(LinearMotionSystem & lms, const math::vec3 & pos, const math::vec3 & axis_h, const math::vec3 & axis_w, size_t size_h, size_t size_w, float step_h, float step_w) : Cloth(lms)
  {
    width = size_w;
    ...
    for (size_t row = 0; row < size_h; row++)
    {
      ...
      {
        math::vec3 pointPos = pos + ((float)row * stepRow) + ((float)col * stepCol);
